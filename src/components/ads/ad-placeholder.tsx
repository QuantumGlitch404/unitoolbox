...
// Common AdSense ad unit types and their typical dimensions
...
interface AdPlaceholderProps {
  ...
  label?: string;
  className?: string;
  type?: keyof typeof adTypeDimensions; // Use keys from our dimensions map
}
...
export function AdPlaceholder({
  ...
  const styleWidth = customWidth ?? dimensions?.width ?? '100%'; // Default to 100% width if no type/custom
  const styleHeight = customHeight ?? dimensions?.height ?? '90px'; // Default to 90px height
